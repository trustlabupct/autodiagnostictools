...
def filter_suspicious_processes(process_logs):
    ...
    for process in process_logs:
        ...
        if (
            (
                exe == r"C:\Windows\syswow64\hh.exe"
                or exe == r"C:\Windows\system32\hh.exe"
            )
            or (exe == r"C:\Windows\syswow64\hh.exe")
            or (exe == r"('C:\\Windows\\syswow64\\hh.exe', '', '', '', '')")
            or (exe == r"('', '', '', '', 'C:\\Windows\\system32\\hh.exe')")
        ):
...
def main():
    ...
    output_file = os.path.join(
        base_dir, "..", "..", "output", "suspicious_results_CAR-2020-11-009.json"
    )
