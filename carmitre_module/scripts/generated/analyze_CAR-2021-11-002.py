...
def filter_suspicious_processes(process_logs):
    ...
    for process in process_logs:
        ...
        if (
            "*Userinit*" in command_line
            or "*Shell*" in command_line
            or "*Notify*" in command_line
            or "*reg*" in command_line
            or "*add*" in command_line
            or ("*Userinit*" in command_line or "*Shell*" in command_line)
            or ("*reg*" in command_line or "*add*" in command_line)
        ):
...
def main():
    ...
    output_file = os.path.join(
        base_dir, "..", "..", "output", "suspicious_results_CAR-2021-11-002.json"
    )
