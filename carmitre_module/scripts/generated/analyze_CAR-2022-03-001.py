...
def filter_suspicious_processes(process_logs):
    ...
    for process in process_logs:
        ...
        if (
            ...
            or "*MiniNt*" in command_line
            or "*EventLog*" in command_line
            or "*EventLog*" in command_line
            or "*reg add*" in command_line
            or "*sc config*" in command_line
            or "*auditpol*" in command_line
            or "*wevtutil*" in command_line
            or "*sl*" in command_line
        ):
...
def filter_suspicious_registry_keys(registry_logs):
    ...
    for reg in registry_logs:
        ...
        key = reg.get("key", "")
        if (key == "EventLog") and (
            value == "Start" or value == "File" or value == "MaxSize"
        ):
...
def main():
    ...
    output_file = os.path.join(
        base_dir, "..", "..", "output", "suspicious_results_CAR-2022-03-001.json"
    )
