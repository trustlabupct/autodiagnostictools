...
def filter_suspicious_processes(process_logs):
    ...
    for process in process_logs:
        ...
        if (exe == "reg.exe" or exe == "cmd.exe") or exe == "reg.exe" or (exe == "cmd.exe") or exe == "cmd.exe":
...
def main():
    ...
    output_file = os.path.join(base_dir, "..", "..", "output", "suspicious_results_CAR-2013-03-001.json")
