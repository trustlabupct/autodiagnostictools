...
def filter_suspicious_processes(process_logs):
    ...
    for process in process_logs:
        ...
        if key == "SOFTWAREMicrosoftWindows" or key == "SOFTWAREWow6432NodeMicrosoftWindows":
...
def filter_suspicious_registry_keys(registry_logs):
    ...
    for reg in registry_logs:
        ...
        value = reg.get("value", "")
        key = reg.get("key", "")
        if "SOFTWAREMicrosoftWindows" in key or "SOFTWAREWow6432NodeMicrosoftWindows" in key:
...
def main():
    ...
    output_file = os.path.join(base_dir, "..", "..", "output", "suspicious_results_CAR-2020-09-005.json")
