...
def filter_suspicious_processes(process_logs):
    ...
    for process in process_logs:
        ...
        if (exe == "w3wp.exe") or exe == "w3wp.exe" or (exe == "httpd.exe") or exe == "httpd.exe" or (exe == "tomcat*.exe") or exe == "tomcat.exe" or (exe == "nginx.exe") or exe == "nginx.exe" or (exe == "cmd.exe") or exe == "cmd.exe" or (exe == "powershell.exe") or exe == "powershell.exe" or (exe == "net.exe") or exe == "net.exe" or (exe == "whoami.exe") or exe == "whoami.exe" or (exe == "hostname.exe") or exe == "hostname.exe" or (exe == "systeminfo.exe") or exe == "systeminfo.exe":
...
def main():
    ...
    output_file = os.path.join(base_dir, "..", "..", "output", "suspicious_results_CAR-2021-02-001.json")
