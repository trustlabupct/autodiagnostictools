...
def filter_suspicious_processes(process_logs):
    ...
    for process in process_logs:
        ...
        if (
            ...
            or (exe == r"csrss.exe")
            or (exe == r"('', '', '', '', 'csrss.exe')")
            or (exe == r"wininit.exe")
            or (exe == r"('', '', '', '', 'wininit.exe')")
            or (exe == r"winlogon.exe")
            or (exe == r"('', '', '', '', 'winlogon.exe')")
            or (exe == r"lsass.exe")
            or (exe == r"('', '', '', '', 'lsass.exe')")
            or (exe == r"LogonUI.exe")
            or (exe == r"('', '', '', '', 'LogonUI.exe')")
            or (exe == r"services.exe")
            or (exe == r"('', '', '', '', 'services.exe')")
            or (exe == r"spoolsv.exe")
            or (exe == r"('', '', '', '', 'spoolsv.exe')")
            or (exe == r"taskhost.exe")
            or (exe == r"('', '', '', '', 'taskhost.exe')")
            or (exe == r"taskhostw.exe")
            or (exe == r"('', '', '', '', 'taskhostw.exe')")
            or (exe == r"userinit.exe")
            or (exe == r"('', '', '', '', 'userinit.exe')")
        ):
...
def main():
    ...
    output_file = os.path.join(
        base_dir, "..", "..", "output", "suspicious_results_CAR-2020-11-004.json"
    )
