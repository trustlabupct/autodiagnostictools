...
def filter_suspicious_registry_keys(registry_logs):
    ...
    for reg in registry_logs:
        ...
        value = reg.get("value", "")
        key = reg.get("key", "")
        if key == r"*\Software\Policies\Microsoft\Windows\Control Panel\Desktop\SCRNSAVE.EXE":
...
def main():
    ...
    output_file = os.path.join(base_dir, "..", "..", "output", "suspicious_results_CAR-2020-11-011.json")
