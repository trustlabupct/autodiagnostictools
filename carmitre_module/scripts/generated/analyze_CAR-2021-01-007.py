...
def filter_suspicious_processes(process_logs):
    ...
    for process in process_logs:
        ...
        if (
            ...
            or ("sc *config*" in command_line or "sc *stop*" in command_line)
            or ("sc *config*" in command_line or "sc *query*" in command_line)
            or (exe == r"('', '', '', '', 'C:\\\\Windows\\\\System32\\\\sc.exe')")
            or "sc *config*" in command_line
            or "sc *stop*" in command_line
            or "sc *query*" in command_line
        ):
...
def main():
    ...
    output_file = os.path.join(
        base_dir, "..", "..", "output", "suspicious_results_CAR-2021-01-007.json"
    )
