...
def filter_suspicious_processes(process_logs):
    ...
    for process in process_logs:
        ...
        if (
            (exe == r"C:\Windows\System32\bcdedit.exe")
            and ("recoveryenabled" in command_line)
            or (exe == r"('', '', '', '', 'C:\\Windows\\System32\\bcdedit.exe')")
            or "*recoveryenabled*" in command_line
        ):
...
def main():
    ...
    output_file = os.path.join(
        base_dir, "..", "..", "output", "suspicious_results_CAR-2021-05-003.json"
    )
