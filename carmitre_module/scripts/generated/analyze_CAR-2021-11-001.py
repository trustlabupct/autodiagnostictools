...
def filter_suspicious_processes(process_logs):
    ...
    for process in process_logs:
        ...
        if (
            "*SafeDllSearchMode*" in command_line
            or "*reg*" in command_line
            or "*add*" in command_line
            or "*00000000*" in command_line
            or ("*reg*" in command_line and "*add*" in command_line)
        ):
...
def main():
    ...
    output_file = os.path.join(
        base_dir, "..", "..", "output", "suspicious_results_CAR-2021-11-001.json"
    )
