...
def filter_suspicious_processes(process_logs):
    ...
    for process in process_logs:
        ...
        if (exe == "hostname.exe") or exe == "hostname.exe" or (exe == "ipconfig.exe") or exe == "ipconfig.exe" or (exe == "net.exe") or exe == "net.exe" or (exe == "quser.exe") or exe == "quser.exe" or (exe == "qwinsta.exe") or exe == "qwinsta.exe" or (exe == "sc") or exe == "sc" or (exe == "systeminfo.exe") or exe == "systeminfo.exe" or (exe == "tasklist.exe") or exe == "tasklist.exe" or (exe == "whoami.exe") or exe == "whoami.exe":
...
def main():
    ...
    output_file = os.path.join(base_dir, "..", "..", "output", "suspicious_results_CAR-2016-03-001.json")
