...
def filter_suspicious_processes(process_logs):
    ...
    for process in process_logs:
        ...
        if (
            (exe == r"('', '', '', '', 'net.exe')")
            or "*net* user*" in command_line
            or "*net* group*" in command_line
            or "*net* localgroup*" in command_line
            or "*get-localgroup*" in command_line
            or "*get-ADPrincipalGroupMembership*" in command_line
        ):
...
def main():
    ...
    output_file = os.path.join(
        base_dir, "..", "..", "output", "suspicious_results_CAR-2020-11-006.json"
    )
