...
def filter_suspicious_processes(process_logs):
    ...
    for process in process_logs:
        ...
        if (
            "*create*" in command_line
            or "*addfile*" in command_line
            or "*setnotifyflags*" in command_line
            or "*setnotifycmdline*" in command_line
            or "*setminretrydelay*" in command_line
            or "*setcustomheaders*" in command_line
            or "*resume*" in command_line
        ) or (exe == r"('', '', '', '', 'C:\\Windows\\System32\\bitsadmin.exe')"):
...
def main():
    ...
    output_file = os.path.join(
        base_dir, "..", "..", "output", "suspicious_results_CAR-2021-05-004.json"
    )
