...
def filter_suspicious_processes(process_logs):
    ...
    for process in process_logs:
        ...
        if (
            "*reg* query HKLM /f password /t REG_SZ /s*" in command_line
            or "reg* query HKCU /f password /t REG_SZ /s" in command_line
            or "*Get-UnattendedInstallFile*" in command_line
            or "*Get-Webconfig*" in command_line
            or "*Get-ApplicationHost*" in command_line
            or "*Get-SiteListPassword*" in command_line
            or "*Get-CachedGPPPassword*" in command_line
            or "*Get-RegistryAutoLogon*" in command_line
        ):
...
def main():
    ...
    output_file = os.path.join(
        base_dir, "..", "..", "output", "suspicious_results_CAR-2020-09-004.json"
    )
