...
def filter_suspicious_processes(process_logs):
    ...
    for process in process_logs:
        ...
        if (
            (exe == r"('tshark.exe', '', '', '', '')")
            or (exe == r"('windump.exe', '', '', '', '')")
            or (exe == r"('', '', '', '', 'logman.exe')")
            or (
                exe == "logman.exe"
                and parent_exe
                and parent_exe
                != r"C:\Program Files\Windows Event Reporting\Core\EventReporting.AgentService.exe"
            )
            or (exe == r"('tcpdump.exe', '', '', '', '')")
            or (exe == r"('wprui.exe', '', '', '', '')")
            or (exe == r"('', '', '', '', 'wpr.exe')")
        ):
...
def main():
    ...
    output_file = os.path.join(
        base_dir, "..", "..", "output", "suspicious_results_CAR-2020-11-002.json"
    )
