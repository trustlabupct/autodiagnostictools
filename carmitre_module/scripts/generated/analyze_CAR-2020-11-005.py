...
def filter_suspicious_processes(process_logs):
    ...
    for process in process_logs:
        ...
        if (
            (
                "*rm (Get-PSReadlineOption).HistorySavePath*" in command_line
                or "*del (Get-PSReadlineOption).HistorySavePath*" in command_line
            )
            or (
                "*Set-PSReadlineOption –HistorySaveStyle SaveNothing*" in command_line
            ...
            or "*rm (Get-PSReadlineOption).HistorySavePath*" in command_line
            or "*del (Get-PSReadlineOption).HistorySavePath*" in command_line
            or "*Set-PSReadlineOption –HistorySaveStyle SaveNothing*" in command_line
            or "*Remove-Item (Get-PSReadlineOption).HistorySavePath*" in command_line
        ):
...
def main():
    ...
    output_file = os.path.join(
        base_dir, "..", "..", "output", "suspicious_results_CAR-2020-11-005.json"
    )
