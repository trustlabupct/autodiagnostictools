...
def filter_suspicious_service_logs(service_logs):
    ...
    for log in service_logs:
        ...
        if "\"\.exe\" AND image_path does not contain [\"C:\\Windows\\\", \"%windir%\\\", \"C:\\Program File\", \"C:\\Programdata\\\", \"%systemroot%\\\"] " in image_path or "C:\\Windows\\*" not in image_path and "%windir%\\*" not in image_path and "C:\\Program File*" not in image_path and "C:\\Programdata\\*" not in image_path and "%systemroot%\\*] )" not in image_path:
...
def main():
    ...
    output_file = os.path.join(base_dir, "..", "..", "output", "suspicious_results_CAR-2021-05-012.json")
