...
def filter_suspicious_processes(process_logs):
    ...
    for process in process_logs:
        ...
        if (
            (
                exe == r"C:\\Windows\\SysWOW64\\mavinject.exe"
                or image == r"C:\\Windows\\System32\\mavinject.exe"
                or "*/INJECTRUNNING*" in command_line
            )
            or (
                exe == r"('C:\\\\Windows\\\\SysWOW64\\\\mavinject.exe', '', '', '', '')"
            )
...
def main():
    ...
    output_file = os.path.join(
        base_dir, "..", "..", "output", "suspicious_results_CAR-2020-11-003.json"
    )
