...
def filter_suspicious_processes(process_logs):
    ...
    for process in process_logs:
        ...
        if key == "EnvironmentUserInitMprLogonScript":
...
def filter_suspicious_registry_keys(registry_logs):
    ...
    for reg in registry_logs:
        ...
        value = reg.get("value", "")
        key = reg.get("key", "")
        if "EnvironmentUserInitMprLogonScript" in key:
...
def main():
    ...
    output_file = os.path.join(base_dir, "..", "..", "output", "suspicious_results_CAR-2020-11-001.json")
