...
def filter_suspicious_processes(process_logs):
    ...
    for process in process_logs:
        ...
        if (exe == "powershell.exe OR rundll32.exe OR wmic.exe OR wscript.exe OR cscript.exe") and re.match(r"__some_regex__", command_line) or exe == "powershell.exe OR rundll32.exe OR wmic.exe OR wscript.exe OR cscript.exe":
...
def main():
    ...
    output_file = os.path.join(base_dir, "..", "..", "output", "suspicious_results_CAR-2020-08-001.json")
