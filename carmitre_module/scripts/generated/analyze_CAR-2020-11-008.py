...
def filter_suspicious_processes(process_logs):
    ...
    for process in process_logs:
        ...
        if (exe == r"C:\Program Files (x86)\Microsoft Visual Studio\*\bin\MSBuild.exe" or exe == r"C:\Windows\Microsoft.NET\Framework*\msbuild.exe") or (exe == r"C:\Program Files (x86)\Microsoft Visual Studio\*\bin\MSBuild.exe") or (exe == r"('C:\\Program Files (x86)\\Microsoft Visual Studio\\*\\bin\\MSBuild.exe', '', '', '', '')") or (exe == r"('C:\\Windows\\Microsoft.NET\\Framework*\\msbuild.exe', '', '', '', '')") or (exe == r"('', '', '', '', 'C:\\users\\*\\appdata\\roaming\\microsoft\\msxsl.exe')"):
...
def main():
    ...
    output_file = os.path.join(base_dir, "..", "..", "output", "suspicious_results_CAR-2020-11-008.json")
