...
def filter_suspicious_processes(process_logs):
    ...
    for process in process_logs:
        ...
        if image_path == "c:windowssystem32fodhelper.exe" or parent_image_path == "c:\windows\system32\fodhelper.exe" or cmd == ".exe\"cleanmgr.exe" or image_path == "c:program" or image_path == "c:windowssystem32slui.exe" or parent_image_path == "c:\windows\system32\slui.exe" or cmd == "'\"c:windowssystem32dism.exe\"\"\".xml\"'" or cmd == "'\"c:windowssystem32wusa.exe\"/quiet'" or image_path == "c:windowsdccw.exe" or parent_image_path == "c:\windows\dccw.exe":
...
def filter_suspicious_service_logs(service_logs):
    ...
    for log in service_logs:
        ...
        if "= \"c:\windows\system32\fodhelper.exe\") or" in image_path or "= \"c:\program files\windows media player\osk.exe\") or" in image_path or "= \"c:\windows\system32\slui.exe\") or" in image_path or "= \"c:\windows\dccw.exe\" and image_path != \"c:\windows\system32\cttune.exe\"" in image_path:
...
def filter_suspicious_file_logs(file_logs):
    ...
    for log in file_logs:
        ...
        image_path = log.get("image_path", "")
        if "c:\users\\appdata\local\temp\\dismhost.exe) or" in image_path or "c:\windows\explorer.exe) or" in image_path or "c:\windows\system32\cttune.exe" in image_path:
...
def main():
    ...
    output_file = os.path.join(base_dir, "..", "..", "output", "suspicious_results_CAR-2019-04-001.json")
