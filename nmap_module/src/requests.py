...
def port_scanner_tcp_syn(hosts_list, port_range):  
    ...
    nmScan.scan(hosts=final_hosts, arguments=f' -sS -Pn --top-ports {port_range}') # Skip discovery phase and process a TCP Syn scan
    scan_results = {}
    ...
    for host in nmScan.all_hosts():
        if 'tcp' in nmScan[host]:
            ...
            for port, port_data in nmScan[host]['tcp'].items():
                if port_data['state'] in ['open', 'filtered']:  # Keep only 'open' or 'filtered'
                    if host not in scan_results:
                        scan_results[host] = {}  # Initialize a subdictionary for each host
                    scan_results[host][str(port)] = port_data['state']  # Add the port and its state
    return scan_results
...
def port_scanner_udp(hosts_list, port_range):
    nmScan = nmap.PortScanner() # Initialize the port scanner
    final_hosts = ' '.join([host[0] for host in hosts_list])
    nmScan.scan(hosts=final_hosts, arguments=f' -sU -Pn --top-ports {port_range}') # Skip discovery phase and process a UDP scan
    scan_results = {}
...
if __name__ == "__main__":
    ...
    if ip_validation == True and port_validation_tcp == True and port_validation_udp == True:
