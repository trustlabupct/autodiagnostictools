...
def checkClamInstallation():
    ...
    try:
        ...
        try:
            ...
            subprocess.run(["apt-get", "install", "-y", "clamav", "clamav-daemon"], check=True)
            subprocess.run(
...
def startClamdWSL():
    ...
    try:
        result = subprocess.run(["pgrep", "clamd"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if result.returncode == 0:
            print("The clamd service is already running.")
        else:
            print("Starting clamd service manually in WSL...")
            result = subprocess.run(["clamd"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            if result.returncode == 0:
...
def scanDirectories():
    ...
    try:
        ...
        for user_dir in user_dirs:
            downloads_directory_windows = os.path.join("C:\\Users", user_dir, "Downloads")
            documents_directory_windows = os.path.join("C:\\Users", user_dir, "Documents")
